...
#ifndef LEARN_CPP_SJSTL_SJDEQUE_H_
#define LEARN_CPP_SJSTL_SJDEQUE_H_
...
struct __deque_iterator {
  ...
  self operator+=(difference_type n) {
    ...
    if (offset >= 0 && offset < difference_type(buffer_size())) { // in one buffer
      cur += n;
    } else {
...
class Deque {
 ...
 protected:
  ...
  pointer allocate_node() {
    pointer tmp = data_allocator::allocate(buffer_size());
    return tmp;
  }
  ...
  void create_map_and_nodes(size_type num_elements) {
    ...
    map_size = std::max(initial_map_size(), num_elements + 2);
    map = map_allocator::allocate(map_size);

    map_pointer nstart = map + (map_size - num_elements) / 2;
  ...
  void reallocate_map(size_type nodes_to_add, bool add_at_front) {
    ...
    start.set_node(new_nstart + old_num_nodes - 1);
  }
...
#endif //LEARN_CPP_SJSTL_SJDEQUE_H_
